  ...
  it('dispatches success action when login succeeds', () => {
    ...
    return store.dispatch(login())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', LOGIN_LOGIN_BEGIN);
        expect(actions[1]).toHaveProperty('type', LOGIN_LOGIN_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when login fails', () => {
    ...
    return store.dispatch(login({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', LOGIN_LOGIN_BEGIN);
        expect(actions[1]).toHaveProperty('type', LOGIN_LOGIN_FAILURE);
